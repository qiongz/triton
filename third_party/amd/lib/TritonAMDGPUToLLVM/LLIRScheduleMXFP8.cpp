...
private:
  ...
  static void moveAnchors(SmallVectorImpl<AnchorInst> &Anchors,
    ...
    // Insert extra users right after their defining LR. The defining LR is
    // whichever operand of EU is in ToMove -- NOT necessarily operand 0 (a
    // microscale transpose-read convert can have a non-Instruction operand 0).
    // Search operands via dyn_cast so the clone-scheduling completes instead of
    // asserting (lets the clone-bail verify+skip unschedulable microscale shapes).
...
// MXFP8-aware LLIR scheduler (TRITON_ENABLE_LLIR_SCHED_MXFP8). Separate from the
// stock runLLIRSchedulePass so a4w4/a8w8/a16w16 (which use the stock pass) are
// byte-identical. Extends the positional scheduler with a dependency-preserving
